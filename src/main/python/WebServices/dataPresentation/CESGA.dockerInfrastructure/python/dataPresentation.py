...
def executeDiseasePredictionGenerator():
    ...
    Optional parameter. Retrieves the number of weeks ago the prediction is retrieved- > date.today() - weeksAgo weeks
    ---
    tags:
      - dataPresentation
    responses:
...
def vineyardsDiseasePrediction(*args, **kwargs):
    ...
    try:
        ...
        logInfo='{}\nNumber of records: {}'.format(logInfo, rGenerateCSVFromDB["count"])
        if config.copyFile == 'True':
