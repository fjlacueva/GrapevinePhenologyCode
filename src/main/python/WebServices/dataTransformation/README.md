...
**NOTES**:
- **To run the program on host**: > sudo $PYTHONENV/python appDataTransformation.py \
...
- **To recreate the environment from the envRequirements.txt file**:
    ```
    python3 -m venv --copies /python/.envs/dataTransformationEnv #copies avoids symlinks to be used
    . ./launchEnv.sh
