...
from flask_futureExecutor.helpers import InstanceProxy
...
class FutureCollection:
    ...
    def key(self, future):
        idx= list(self._futures.values()).index(future)
        if idx >= 0:
            return list(self._futures.keys())[idx]
        return None
    ...
    def get(self, future_key):
        """Return a Future. Futures that are
        ready to be used should always be popped so they do not continue to
        consume memory.
...
class FutureProxy(InstanceProxy, Future):
    ...
    :param executor: An instance of :class:`flask_FutureExecutor.Executor` which
